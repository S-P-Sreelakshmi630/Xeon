...
const RecentTransactions = ({ accounts,
  ...
  <div>
    <h2 className="recent-transactions-heading recent-transactions-label">Recent Transactions</h2>
    <div className="table-responsive "> {/* Added to make the table horizontally scrollable on smaller screens */}
        ...
        <tbody>
          {transactions.map(transaction => (
            <tr key={transaction.accountId}>
              ...
              <td className={transaction.amount >= 0 ? 'text-success' : 'text-danger'}>${Math.abs(transaction.amount).toFixed(2)}</td>
              <td>{transaction.description}</td>
              <td className={transaction.status=='success'?'text-success': 'text-danger'}>{transaction.status}</td>
              <td>
